...
# This is my code for the "Choose your own" part of the Capstone
# I created different models to try to predict the Default (the fail in payment of a credit card debt)
# The Data is a small sample provided by a financial company from Chile.

# Hope you enjoy it, for me it was quite fun. 
# I think there is room for improvement, but I am happy with the things I learned. So, lets start.

# In First place, I load the required packages:
...
# The DataBase has 12 columns. 11 independent variables (X) and 1 dependent (Y). The dependent variable is a flag 
# that shows if the person has paid the credit card debt or not. It is a binary variable.
ncol(DataBase)


## UNIVARIATE ANALYSIS OF THE INDEPENDET VARIABLES.
# Let's study the distribution of the independent variables, a simple way to do this is with a histogram:

# The first variable is "Commercial References". It shows the amount of times the person under analysis
# did not pay some expenses like electricity and cellphone bills (here in Chile you can buy this type of 
# information).
...
# The third variable is an interesting one, it is "Basic Needs". It shows the percentage of basic needs
# that are covered in the region the person lives (it can be a huge indicator for the good or bad 
# behavior in payments).

hist(DataBase$BN)

# It is distributed around 91 (it's mean). We can also see a low variance, and it is almost impossible to
# find an observation below the 60 points. In this case, it could be suitable to standardize the variable, 
...
# the data to 18 years old (the legal age when you become an adult).
...
# JUB = (Retired person), UNIV = (University teacher) , ST = (Worker radicated in "Santiago de Chile").
...
hist(Wage_LN)
# The ninth variable is a flag that shows if the person has used a passive product (like a debit card) 12 month
# in a row. (It could be a good indicator of how much use the person gives to the bank's products)  
hist(DataBase$M12_PP)

# The tenth variable is the limit the person has in his/her credit card in the financial sector in Chile.
hist(DataBase$CCLim)
...
# Let's now start with the different models that will be tested. I have create and test 5 different models:
...
ROC_lm_null
#
# This model does not explain the default. A KS = 0 and a ROC = 50 are similar to an aleatory 
# decision between default and no default
...
# This model is much better than the one with only the intercept. Let's try to improve it with a logistic model.
...
KS_glmL_null
ROC_glmL_null
# The same conclusion as the first model, a model with only an intercept does not explain the default.
...
# A model with KS above 60 and ROC above 80 can be consider as a good one. 

# Best KS:
results$model[which.max(results$KS)]

# Best ROC:
results$model[which.max(results$ROC)] 

# In this particular case, we have 2 acceptable models: Decision trees and Multivariate logistic. The best model is 
# Multivariate Logistic Model it has KS = 61.13 and ROC = 88.57 it  can be consider as a very good model and it can do a 
# really good job discriminating defaults.
